!! NEWUOA-P -Pengcheng Xie supervised by Prof. Ya-xiang Yuan
!! 
!! All rights reserved. 
!! 
!!  Pengcheng Xie, 07/05/2021
!!  LSEC, AMSS, Institute of Computational Mathematics and Scientific/Engineering Computing Academy of Mathematics and Systems Science
...
MODULE newuoa_module
  ...
  contains
   ...
   SUBROUTINE newuoa_min(CALFUN,CALGREY,NF, X, RHOBEG, RHOEND, IPRINT, MAXFUN,PROBLEM)
      implicit none
      interface
         ...
         subroutine CALGREY(i_x, i_k, o_g,i_flag,i_eps1,I_eps2,i_rand,i_problem)
            real(kind=8), dimension(:) :: i_x
            real(kind=8) :: o_g
            integer(kind=4) :: i_k
            integer(kind=4) :: i_flag
            double precision,dimension(:) :: i_eps1,i_eps2,i_rand
           character(len=15) :: i_problem 
         end subroutine
      end interface
   ...
   END SUBROUTINE newuoa_min
      ...
      SUBROUTINE NEWUOA(CALFUN,CALGREY,NF,N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W,PROBLEM)
      implicit none
...
!     W plus the space that is needed by the last array of NEWUOB.
      ...
      CALL NEWUOB(CALFUN,CALGREY,NF, N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),  &
        ...
        W(IZMAT),NDIM,W(ID),W(IVL),W(IW),PROBLEM)
   20 RETURN
      ...
      SUBROUTINE NEWUOB(CALFUN,CALGREY,NF,N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE, &
        XOPT,XNEW,XPT,FVAL,GQ,HQ,PQ,BMAT,ZMAT,NDIM,D,VLAG,W,PROBLEM)
      ...
      real(kind=8) :: HALF, ONE, TENTH, ZERO, RHOSQ, RECIP,RECIQ, XIPT, &
                     ...
                     DSTEP, SUMZ, VQUAD, G, G1, G2, AVER, VAR
      ...
      logical :: EXIST
      real*8,allocatable :: EPS1(:),EPS2(:),RAND(:),G_XBEG_K(:)

      ALLOCATE(EPS1(MAXFUN))
      call random_seed
      call random_number(EPS1)

      ALLOCATE(EPS2(MAXFUN))
      call random_seed
      call random_number(EPS2)

      ALLOCATE(RAND(MAXFUN))
      call random_seed
      ...
      ALLOCATE(G_XBEG_K(MAXFUN))
      call random_seed
      call random_number(G_XBEG_K)
      DO I=1,MAXFUN
      G_XBEG_K(I)=0.0D0
      END DO
...
!           PRINT 3321, (XOPT(I),I=1,N)
! 3321      FORMAT (/4X,'XOPT1-test =',1PD23.15)
!           PRINT 3329, (D(I),I=1,N)
! 3329      FORMAT (/4X,'D1-test =',1PD23.15)
!           PRINT 3331, (GQ(I),I=1,N)
! 3331      FORMAT (/4X,'GQ1-test =',1PD23.15)
!           PRINT 3332, (PQ(I),I=1,NPT)
! 3332      FORMAT (/4X,'PQ1-test =',1PD23.15)
!           PRINT 3333, (HQ(I),I=1,NH)
! 3333      FORMAT (/4X,'HQ1-test =',1PD23.15)
!           PRINT 3334, DELTA
! 3334      FORMAT (/4X,'DELTA1-test =',1PD23.15)
...
!           PRINT 2262, KNEW,DSQRT(DSQ),RHO,DNORM
! 2262      FORMAT (/4X,'KNEW-test-0405 =',I6,/4X,'DSQ-test-0405 =',1PD23.15 &
!            ,/4X,'RHO-test-0405 =',1PD23.15 ,/4X,'DNORM-test-0405 =',1PD23.15)
    
      IF (DNORM .LT. HALF*RHO) THEN! (0.60D0+(0.5D0)**NF)*RHO) THEN!HALF*RHO) THEN
          KNEW=-1


!                 PRINT 2263, KNEW
! 2263      FORMAT (/4X,'KNEW-test-gugu-0405 =',I6)
...
!           PRINT 2261, TEMP
! 2261      FORMAT (/4X,'TEMP-test-0405 =',1PD23.15)
...
!            ' called MAXFUN times.')
          GOTO 530
      END IF
      CALL CALFUN(X,F,PROBLEM)

      IF (NF .GE. NPT+1) THEN
          CALL CALGREY(X,NF-1,G2,FLAG,EPS1,EPS2,RAND,PROBLEM)
          F=F+G2
...
!  226      FORMAT (/4X,'XBEG-in-test =',1PD23.15)
 ...
 END DO
          ...
          CALL CALGREY(XBEG,NF-2,G1,FLAG,EPS1,EPS2,RAND,PROBLEM)
          CALL CALGREY(XBEG,NF-1,G2,FLAG,EPS1,EPS2,RAND,PROBLEM)
          FBEG=FBEG+(G2-G1)
          G_XBEG_K(NF-1)=100*(XBEG(1)-1)**2*EPS2(NF-1)
 !         PRINT 227, (XBEG(I),I=1,N)
 ! 227      FORMAT (/4X,'XBEG-out-test =',1PD23.15)

!           PRINT 2211, G1
! 2211      FORMAT (/4X,'G1-test =',1PD23.15)
!           PRINT 2212, G2
! 2212      FORMAT (/4X,'G2-test =',1PD23.15)

          DO I=1,N
              XTEMP(I)=XOPT(I)+XBASE(I)
          END DO
          CALL CALGREY(XTEMP,NF-2,G1,FLAG,EPS1,EPS2,RAND,PROBLEM)
          CALL CALGREY(XTEMP,NF-1,G2,FLAG,EPS1,EPS2,RAND,PROBLEM)
          FOPT=(G2-G1)+FOPT
...
!  276      FORMAT (/4X,'XTEMP-test =',1PD23.15)
          ...
          END IF 
      ELSE
          CALL CALGREY(X,NPT,G1,FLAG,EPS1,EPS2,RAND,PROBLEM)
          F=F+G1
...
!          PRINT 222, (FVAL(I),I=1,NPT)
!  222      FORMAT (/4X,'FVAL-test =',1PD23.15)
! 
!          PRINT 223, FBEG
!  223      FORMAT (/4X,'FBEG-test =',1PD23.15)
! 
!          PRINT 224, F
!  224      FORMAT (/4X,'F-test =',1PD23.15)
!     
!          PRINT 225, FOPT
!  225      FORMAT (/4X,'FOPT-test =',1PD23.15)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      IF (NF .GT. (10+2*N))THEN
      AVER=0.0D0
      DO I=(NF-10),(NF-1)
          AVER=AVER+G_XBEG_K(I)
      END DO
      AVER=AVER/10.0D0
      VAR=0.0D0
      DO I=(NF-10),(NF-1)
          VAR=VAR+(G_XBEG_K(I)-AVER)**2
      END DO
      VAR=VAR/10.0D0
      
      INQUIRE(FILE='VAR.csv', EXIST=EXIST)
      IF (EXIST) THEN
          OPEN(2,FILE='VAR.csv', STATUS='old', POSITION='append', ACTION='write')
      ELSE
          OPEN(2,FILE='VAR.csv', STATUS='new', ACTION='write')
      END IF
      WRITE(2,*)VAR
      CLOSE(2)


      IF (VAR .LT. 0.0D0) GOTO 530
      END IF
...
!      IF (NF .EQ. NPT+1) THEN
!      DO I=1,N
!          XTEMP(I)=XOPT(I)+D(I)+XBASE(I)
!      END DO
!      CALL CALGREY(XTEMP,NF-1,G1)
!      CALL CALGREY(XTEMP,NF,G2)
!           PRINT 221, (XOPT(I),I=1,N)
!  221      FORMAT (/4X,'XOPT-test =',1PD23.15)
!           PRINT 229, (D(I),I=1,N)
!  229      FORMAT (/4X,'D-test =',1PD23.15)
!           PRINT 231, (GQ(I),I=1,N)
!  231      FORMAT (/4X,'GQ-test =',1PD23.15)
!           PRINT 232, (PQ(I),I=1,NPT)
!  232      FORMAT (/4X,'PQ-test =',1PD23.15)
!           PRINT 233, (HQ(I),I=1,NH)
!  233      FORMAT (/4X,'HQ-test =',1PD23.15)
!
!
!
      ...
      IF (NF .GE. NPT+1) THEN 
          CALL CALGREY(XTEMP,NF,G2,FLAG,EPS1,EPS2,RAND,PROBLEM)
          CALL CALGREY(XTEMP,NF-1,G1,FLAG,EPS1,EPS2,RAND,PROBLEM)
          DIFF=F-FOPT+(G2-G1)-VQUAD
      END IF

!           PRINT 235, DIFF,F,FOPT,VQUAD,G2,G1
!  235      FORMAT (/4X,'DIFF-0405 =',1PD23.15 & 
!          ,/4X,'F-0405 =',1PD23.15  & 
!           ,/4X,'FOPT-0405 =',1PD23.15  &
!          ,/4X,'VQUAD-0405 =',1PD23.15 &
!          ,/4X,'G2-0405 =',1PD23.15 &
!          ,/4X,'G1-0405 =',1PD23.15)
!           PRINT 371, (XOPT(I),I=1,N)
!  371      FORMAT (/4X,'XOPT0405=',1PD23.15)



      DIFFC=DIFFB
      DIFFB=DIFFA
      DIFFA=DABS((-G2+G1)+DIFF)




!           PRINT 2905, DIFFA,DIFFB,DIFFC
! 2905      FORMAT (/4X,'DIFFA-0405 =',1PD23.15 & 
!          ,/4X,'DIFFB-0405 =',1PD23.15  & 
!           ,/4X,'DIFFC-0405 =',1PD23.15)
...
!          PRINT 361, (XOPT(I),I=1,N)
!  361      FORMAT (/4X,'XOPT++++-test=',1PD23.15)
!          PRINT 351, VQUAD
!  351      FORMAT (/4X,'VQUAD-test=',1PD23.15)
!          PRINT 331
!  331      FORMAT (/4X,'++++++++++++++++++++++END++++++++++++++++++++')
!          PRINT 332
!  332      FORMAT (/4X,'+++++++++++++++++++++BEGIN+++++++++++++++++++')
...
!     vector of the model. Also include the new interpolation point.
                ...
                DO 440 K=1,NPT
  440 PQ(K)=PQ(K)+TEMP1(JJ)*ZMAT(K,J)
...
!                        END DO
!                        CALL CALGREY(XTEMP,NF-1,G2)
!                        CALL CALGREY(XTEMP,NF-2,G1)
                END IF
                IF (J .NE. KNEW) THEN
                        DO K=1,N
                        XTEMP(K)=XBASE(K)+XPT(J,K)
...
!                        PRINT 521, RHO,DELTA
!  521         FORMAT (4X,'kankan RHO:',1PD23.15,9X,/,4X, &
!                'kankan Delta:',1PD23.15)
...
!                   PRINT 5333, (EPS(I),I=1,MAXFUN)
! 5333      FORMAT (/4X,'EPS-test =',1PD23.15)
...
!    print 556,NF
!  556     FORMAT (/4X,'NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNFFFFFFFFFFFFFFFFFFFFFFF =',I6)
...
!           PRINT 550, (EPS1(I),I=1,MAXFUN)
!  550     FORMAT (/4X,'EPS2=',1PD23.15)
!         PRINT 520, F,(X(I),I=1,N)
...
!          print*,PROBLEM,IP
          ...
          CALL newuoa_min(CALFUN,CALGREY,NF,X, RHOBEG=1.0D0, RHOEND=1.0D-6, IPRINT=3, &
              MAXFUN=10000,PROBLEM=a(IP))

          PRINT 550, IP
  550     FORMAT (/4X,'PROBLEM:',I6)
...
!G=X(1)**2+X(2)**2
!F=X(1)**4+X(2)**4
          WRITE(1,*)NF!,F+100.0D0*G,X(:)

      END DO
      CLOSE(1)
END PROGRAM

subroutine CALFUN(X,F,PROBLEM)
      integer*4 :: N,INFO,I
      real*8 :: X(:),F
      character(len=15) :: PROBLEM
      N=size(X)
...
subroutine CALGREY(X,K,G,FLAG,EPS1,EPS2,RAND,PROBLEM)
      implicit real*8 (a-h,o-z)
      ...
      IF (K .GT. 2*N+1)THEN
         ...
         Epsilonk=EPS2(K)
         G=100.0D0*G+EPS2(K)*(100.0D0*G+FF)
      ELSE
      G=100.0D0*G+EPS1(K)
      END IF
      END IF 
...
subroutine Laplace(beta,Epsilonk)
    implicit none
